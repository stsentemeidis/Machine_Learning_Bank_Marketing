#Bivarite Analysis Num Variables
...
list <- unlist(lapply(bank_train, is.numeric))  

for (feature in names(bank_train)[sapply(bank_train, is.numeric)]) {
  mu <- ddply(bank_train, "y", summarise, grp.mean=mean(feature))
  head(mu)
  ggplot(data = bank_train, aes(x = bank_train[ ,feature], color = y, group = y, fill = y)) + 
    geom_density(alpha=0.4)+
    geom_vline(data=mu, aes(xintercept=grp.mean, color=y), linetype = "longdash")+
    theme_minimal()+
...
#tu put in the Shiny App

mu <- ddply(bank_train, "y", summarise, grp.mean=mean(age))
head(mu)
ggplot(data = bank_train, aes(x = age, color = y, group = y, fill = y)) + 
  geom_density(alpha=0.4)+
  geom_vline(data=mu, aes(xintercept=grp.mean, color=y), linetype = "longdash")+
  theme_minimal()+
  theme(panel.grid.major = element_blank(), 
...
# mu <- ddply(bank_train_plot, "y", summarise, grp.mean=mean(age))
# head(mu)
# 
# ggplot(data = bank_train_plot, aes(x = age, color = y, group = y, fill = y)) + 
#   geom_density(alpha=0.4)+
#   geom_vline(data=mu, aes(xintercept=grp.mean, color=y), linetype = "longdash")+
#   theme_minimal()+
#   theme(panel.grid.major = element_blank(), 
