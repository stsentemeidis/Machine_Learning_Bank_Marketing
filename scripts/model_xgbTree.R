...
pipeline_xgbTree <- function(target, train_set, valid_set, test_set,
         ...
         as.data.frame(
           cbind(
             ...
             'F1 Score' = F1_Score(y_pred = get(paste0('pred_xgbTree', suffix)),
                                   y_true = valid_set[,target]),
             'Coefficients' = length(get(paste0('fit_xgbTree', suffix))$finalModel$coefficients),
  ...
  assign(paste0('real_results', suffix), as.data.frame(cbind(
    'Accuracy' = Accuracy(y_pred = get(paste0('submission_xgbTree_valid', suffix))[, target], y_true = as.numeric(valid_set[, c(target)])),
    'Sensitivity' = Sensitivity(y_pred = get(paste0('submission_xgbTree_valid', suffix))[, target], y_true = as.numeric(valid_set[, c(target)])),
    'Precision' = Precision(y_pred = get(paste0('submission_xgbTree_valid', suffix))[, target], y_true = as.numeric(valid_set[, c(target)])),
    'Recall' = Recall(y_pred = get(paste0('submission_xgbTree_valid', suffix))[, target], y_true = as.numeric(valid_set[, c(target)])),
    'F1 Score' = F1_Score(y_pred = get(paste0('submission_xgbTree_valid', suffix))[, target], y_true = as.numeric(valid_set[, c(target)])),
    'Coefficients' = length(get(paste0('fit_xgbTree', suffix))$finalModel$coefficients),
    'Train Time (min)' = round(as.numeric(get(paste0('time_fit_duration', suffix)), units = 'mins'), 1)
  )), envir = .GlobalEnv)
